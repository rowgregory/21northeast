...
export const changelogData: ChangelogEntry[] = [
  {
    ...
    changes: [
      {
        type: 'feature',
        title: 'Cross-Site Listing Sync for CORE',
        description:
          "Added an EileenListing cache table, a daily cron job (sync-eileen-listings) that pulls active listings from Repliers and upserts them, and a public /api/public/eileen-listings endpoint with CORS scoped to coastalreferralxchange.com — allowing CORE to display Eileen's current listings without duplicating Repliers credentials or logic.",
        impact: 'high'
      },
      {
        type: 'bugfix',
        title: 'Prisma v7 Schema Validation Error',
        description:
          'Removed the url field from the datasource block in schema.prisma — Prisma v7 no longer permits a connection URL in the schema file at all, requiring it exclusively in prisma.config.ts. Fixes "Prisma schema validation (get-config wasm)" error P1012 on build.',
        impact: 'high'
      },
      {
        type: 'refactor',
        title: 'Removed Legacy next-sitemap Postbuild Step',
        description:
          'Confirmed removal of the next-sitemap postbuild script, config file, and package dependency now that app/sitemap.ts and app/robots.ts fully own sitemap and robots.txt generation — eliminating the risk of a stale static sitemap overwriting the dynamic, listing-aware one at deploy time.',
        impact: 'medium'
      }
    ]
  },
  {
    ...
    changes: [
      ...
      {
        type: 'bugfix',
        title: 'Prisma v7 CLI Configuration',
        description:
          'Added prisma.config.ts to define the datasource URL, resolving "The datasource.url property is required" errors on prisma db push and prisma generate introduced by the Prisma v7 CLI changes.',
        impact: 'high'
      },
      {
        type: 'improvement',
        title: 'LogAction Type Extended',
        description:
          'Added CRON_SYNC_EILEEN_LISTINGS to the LogAction union so the new sync cron job can log success/failure through the existing createLog utility.',
        impact: 'low'
      },
      {
        type: 'ui',
        title: 'Favicon Refresh — White on Transparent',
        description:
          'Regenerated favicon.ico and all derived icon sizes using a pure white C21 mark on a fully transparent background, sized to fill the canvas edge-to-edge instead of the previous padded dark-background version.',
        impact: 'low'
      },
      {
        type: 'improvement',
        title: 'Footer Areas We Serve Section',
        description:
          'Added a fourth footer column linking directly to all 11 town pages, rebalanced the footer grid to accommodate it, and fixed footer visibility to include /areas and every *-realtor route.',
        impact: 'medium'
      }
    ]
  },
